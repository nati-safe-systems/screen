...
(function (root) {
  ...
  function css(){
    var pad = (T.pad != null ? T.pad : (T.band + 2.6)).toFixed(2);
    var unit = HOST ? '%' : 'vh';
    var padCss = HOST
    ...
    return [
      ...
      T.dbl ? '#nsf-frame .band2{inset:'+(T.band+1.1)+'vh!important;opacity:1!important}' : '',
      ...
      T.thin
        ? '.orn{position:relative;border-radius:0;background:none;'+
          'box-shadow:0 0 0 .12vh '+T.c2+';}'
        : T.dbl
        ? '.orn{position:relative;border-radius:0;'+
          'background-image:linear-gradient(160deg,'+T.bg1+','+T.bg2+');'+
          'box-shadow:0 0 0 .1vh '+T.c2+',0 0 0 .5vh rgba(0,0,0,0),'+
          '0 0 0 .62vh '+T.c2+';}'
        : T.soft
        ? '.orn{position:relative;border-radius:2.2vh;'+
          'background-image:radial-gradient(ellipse at 50% 0%,'+T.bg1+','+T.bg2+');'+
          'box-shadow:0 0 3vh '+T.c3+' inset,0 0 2.4vh rgba(179,85,127,.28),'+
          '0 1.2vh 3.4vh rgba(0,0,0,.6);}'
        : '.orn{position:relative;border-radius:0;'+
          'background-image:linear-gradient(160deg,'+T.bg1+','+T.bg2+');'+
      ...
      (T.thin||T.dbl||T.soft) ? '.orn::before{display:none}' : '',
      ...
      (T.thin||T.dbl||T.soft) ? '.orn::after{display:none}' : '',
