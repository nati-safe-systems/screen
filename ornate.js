...
(function (root) {
  ...
  function css(){
    ...
    return [
      ...
      '#nsf-frame .band2{position:absolute;inset:'+(T.band+.5)+'vh;',
        'border:.2vh solid '+T.c2+';opacity:.75;border-radius:.3vh}',
      ...
      '.orn{position:relative;border-radius:1.4vh;',
      ...
      '.orn::before{content:"";position:absolute;inset:-.34vh;border-radius:1.7vh;padding:.34vh;',
      ...
      '.orn::after{content:"";position:absolute;inset:.85vh;border-radius:.8vh;',
  ...
  function apply(opt) {
    ...
    addCorners(f, opt.outer || 13);

    (opt.panels||[]).forEach(function(sel){
      var l=document.querySelectorAll(sel);
      for (var i=0;i<l.length;i++) decorate(l[i], opt.panel||5.5);
    });
